...
namespace UmbracoPortfollio.Logic.Helpers
{
    public static class HtmlHelpers
    {
        ...
        public static Image GetHeaderImage(this IPublishedContent content)
        {
            var umbraco = new UmbracoHelper(UmbracoContext.Current);
            if (content != null && content.HasValue("image"))
            {
                var imageId = content.GetPropertyValue("image");
            ...
            {
                var imageId= umbraco.AssignedContentItem.GetPropertyValue("pageImage");
                var image = umbraco.TypedMedia(imageId);
                return new Image(image);
            }
        }        
